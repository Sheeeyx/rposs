...
import useUpdateTermsCondition from "../../../../services/mutation/terms/terms-and-conditions/update/use-update";
import { useGetDetailsTermsCondition } from "../../../../services/queries/use-get-detail-terms-conditions";

const { TextArea } = Input;

export const TermsConditionUpdate: React.FC = () => {
    const { mutate, isLoading } = useUpdateTermsCondition();

    const navigate = useNavigate();

    const params = useParams();

    const detail = useGetDetailsTermsCondition(params.id as string);
    ...
    const handleSubmit = (data: any, id: string | undefined) => {
        mutate(
            ...
            {
                onSuccess: () => {
                    queryClient.invalidateQueries(`terms-condition${id}`);
