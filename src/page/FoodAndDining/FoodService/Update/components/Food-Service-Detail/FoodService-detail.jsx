...
import useUpdateFoodServiceDetail from '../../../../../../services/mutation/food-and-dining/food-service/update/detail/use-update-detail';
const { Option } = Select;

const { Title } = Typography;


export const FoodServiceDetail = ({data}) => {

    const params = useParams();
    const { mutate } = useUpdateFoodServiceDetail();
    ...
    const onFinish = (data, id) => {
        mutate(
            ...
            {
                onSuccess: () => {
                    queryClient.invalidateQueries(`detail-food-service${params.id}`);
        ...
        <div>
            <Form onFinish={(data) => onFinish(data, params.id)}>
                    <Row justify="start" gutter={30}>
                        <Col span={9}>
                            <FormItem
                                label={"Name of your  food service"}
                                name="name"
                                ...
                                rules={[
                                    {
                                        required: true,
                                        message:
                                            "Please enter name of your food service",
                                    },
                                ]}
                            >
                                <Input />
                    ...
                    >
                        <Col span={9}>
                            ...
                            <Col span={9}>
                                <p className="type">Food Service type :</p>
                                <Select 
                    ...
                    <Row style = {{margin: "18px 0"}}>
                        <FormItem>
                           ...
                           <UploadImg setImgIdList = {setCertificateImg} files = {[data.certificate_file]} onesType cropSize = {1.53 / 1}/>
                        </FormItem>
                    </Row>
